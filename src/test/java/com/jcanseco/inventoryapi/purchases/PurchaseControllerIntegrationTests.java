...
public class PurchaseControllerIntegrationTests {
    ...
    public void receivePurchaseStatusShouldBeNoContent() throws Exception {
        var purchaseId = 1L;
        var receiveDto  = ReceivePurchaseDto.builder()
                        .purchaseId(purchaseId).comment("Received thanks")
                        .build();
        mockMvc.perform(
                        put("/api/purchases/receive")
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(mapper.writeValueAsString(receiveDto))
                )
                .andDo(print())
                .andExpect(status().isNoContent());
